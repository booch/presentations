...
# Vertically Sliced MVP

![Delightful, Usable, Valuable, Feasible - an MVP should include them all](DVUF.jpg)

???

-----

* Image from [Huong Ngo](https://www.slideshare.net/HuongNgoPMP/actionable-metrics-in-lean-product-development)
...
# NOT Vertically Sliced

![Mona Lisa built all at once, in pieces](pieces-of-mona-lisa.jpg)

???

-----

* Image from [Jeff Patton](https://www.slideshare.net/jeffpatton/user-story-mapping-discovery-the-whole-story)
...
![Mona Lisa built in an incremental way](incremental-mona-lisa.jpg)
...
* Ask for values when you need them, not before
    * Memoize if necessary
* Use more declarative terms
    * Don't use `compute` or `get` methods

???

-----

* Caveat for pre-computing: sometimes you *do* need to do it (but not often)
* Using `get` or `compute` - don't pretend that you're telling instead of asking
* Memoization: compute something, but save the result for any later requests
* Don't say `getSum`, just say `sum`
