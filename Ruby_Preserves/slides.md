...
* Today we're going to talk about ORMs in general
    * And one I've been writing in particular
...
UserRepository = Preserves.repository(model: User) do
  ...
  def all
    ...
    addresses = repository.select("SELECT * FROM addresses")
    users.to_objects(mapper, addresses: addresses)
  end
end
~~~
